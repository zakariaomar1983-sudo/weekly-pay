...
const driversChannel =
  typeof BroadcastChannel !== "undefined"
    ? new BroadcastChannel("opx-drivers")
    : null;
...
function setDriversSyncStatus(message, tone = "neutral", { at = Date.now(), persist = true } = {}) {
  ...
  if (persist) {
    localStorage.setItem(
      DRIVERS_SYNC_STATUS_KEY,
      JSON.stringify({ message, tone, at })
    );
  }

  window.dispatchEvent(
    new CustomEvent("opx:sync-health-change", {
      detail: { source: "Drivers", message, tone, at }
    })
  );
...
function restoreDriversSyncStatus() {
  ...
  setDriversSyncStatus(saved.message, saved.tone || "neutral", {
...
function updateDriversQueueBanner() {
  ...
  if (navigator.onLine === false) {
    setDriversQueueBanner(
      "Offline mode: driver changes are saving on this device and will sync automatically when internet returns.",
      "offline",
      true
    );
    return;
  ...
  if (driverRetryAttempt || driverRetryTimerId || driverSyncQueued) {
    setDriversQueueBanner(
      "Sync queue active: driver changes are saved locally and retrying automatically in the background.",
      "pending",
      true
    );
    return;
...
function queueDriverSyncRetry(errorMessage = "") {
  ...
  const delay =
    DRIVER_SYNC_RETRY_DELAYS_MS[
      Math.min(driverRetryAttempt, DRIVER_SYNC_RETRY_DELAYS_MS.length - 1)
    ];
...
function normalizeDriverRow(row) {
  ...
  return {
    ...
    name: cleanDriverName(firstValue(raw, ["name", "driverName", "driver", "fullName", "driver_name"]) || ""),
...
function getDriverAttachments(recordId) {
  return Array.isArray(driverAttachmentStore?.[recordId])
    ? driverAttachmentStore[recordId]
    : [];
}
...
function drawDriverAttachments(recordId = currentDriverRecordId()) {
  ...
  if (!recordId) {
    list.innerHTML =
      "<p class='muted'>Save or start a driver record, then attach licence, ID, or contract files.</p>";
  ...
  if (!attachments.length) {
    ...
    setDriverAttachmentStatus(
      `Up to ${DRIVER_ATTACHMENT_LIMIT} files, max ${formatAttachmentSize(DRIVER_ATTACHMENT_MAX_BYTES)} each.`
    );
    return;
  }
  ...
  setDriverAttachmentStatus(
    `${attachments.length}/${DRIVER_ATTACHMENT_LIMIT} documents saved for this driver.`
  );
}
...
async function addDriverAttachments(fileList) {
  ...
  for (const file of files) {
    ...
    if (Number(file.size || 0) > DRIVER_ATTACHMENT_MAX_BYTES) {
      messages.push(
        `${file.name} skipped. Files must be under ${formatAttachmentSize(DRIVER_ATTACHMENT_MAX_BYTES)}.`
      );
      continue;
    }
  ...
  setDriverAttachmentStatus(
    messages.length
      ? messages.join(" ")
      : `${current.length} driver document${current.length === 1 ? "" : "s"} ready.`
  );
...
function removeDriverAttachment(recordId, attachmentId) {
  ...
  driverAttachmentStore[recordId] = current.filter(
...
function openDriverAttachment(recordId, attachmentId) {
  const attachment = getDriverAttachments(recordId).find(
...
function saveData() {
  ...
  window.dispatchEvent(
    new CustomEvent("opx:drivers-updated", {
      detail: { updatedAt }
    })
  );
...
async function syncDriversToSupabase() {
  ...
  try {
    if (!rows.length) {
      const wipe = await driversSupabase
        .from(DRIVERS_TABLE)
        .delete()
        .not("id", "is", null);
    ...
    const { error } = await driversSupabase
      .from(DRIVERS_TABLE)
    ...
    const cleanup = await driversSupabase
      .from(DRIVERS_TABLE)
      .delete()
      .not("id", "in", inList);
...
async function hydrateDriversFromSupabase() {
  ...
  const { data, error } = await driversSupabase
    .from(DRIVERS_TABLE)
    .select("*");
  ...
  if (error) {
    ...
    updateDataStatus(
      `Shared driver data could not load: ${error.message}. Showing this device's saved driver data.`
    );
    setDriversSyncStatus(
      "Shared driver sync unavailable. Using this device's saved data.",
      "local"
    );
    return;
  }
...
function drawStats() {
  const active = state.drivers.filter((driver) => driver.status === "Active").length;
  const onLeave = state.drivers.filter((driver) => driver.status === "On Leave").length;
  ...
  grid.innerHTML = stats
    .map((stat) => `<article class='stat-card'><p>${stat.label}</p><h3>${stat.value}</h3></article>`)
    .join("");
}

function drawTable() {
  const tbody = document.getElementById("driversTableBody");
  const query = (document.getElementById("driversSearch")?.value || "")
    .trim()
    .toLowerCase();
  ...
  if (!filtered.length) {
    tbody.innerHTML = `<tr><td colspan='6' class='empty'>${
      query ? "No drivers match your search." : "No drivers yet."
    }</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = filtered
    ...
    .map((item) => {
      ...
      return `<tr>
        <td>
          <strong title="${item.name || "Unnamed driver"}">${item.name || "Unnamed driver"}</strong>
          <div class="attachment-summary">
            ${attachmentCount ? `<span class="ack-chip ack-chip-neutral">${attachmentCountLabel(attachmentCount)}</span>` : ""}
          </div>
...
function applyAccessControl() {
  ...
  if (!auth.can("accessLogs")) {
    const logLink = document.querySelector("a[href='./log.html']");
...
document.getElementById("driversForm").addEventListener("submit", (event) => {
  ...
  const payload = {
    id: id || uid(),
    name: cleanDriverName(document.getElementById("driverDetailsName").value),
  ...
  const duplicate = state.drivers.find(
    (driver) =>
      driver.id !== payload.id &&
      normalizeDriverNameKey(driver.name) === normalizeDriverNameKey(payload.name)
  );

  if (duplicate) {
    const shouldContinue = confirm(
      `${payload.name} already exists. Do you want to save another driver with the same name?`
    );
  ...
  state.drivers = id
    ? state.drivers.map((driver) => (driver.id === id ? payload : driver))
...
document.getElementById("cancelDriverEdit").addEventListener("click", () => {
  ...
  if (
    draftId &&
    !state.drivers.some((driver) => driver.id === draftId) &&
    driverAttachmentStore[draftId]
  ) {
...
document.getElementById("driverAttachmentsInput")?.addEventListener("change", async (event) => {
  ...
  try {
    ...
    setDriverAttachmentStatus(
      error.message || "Could not add the selected driver document.",
      "error-text"
    );
  } finally {
    input.value = "";
...
document.body.addEventListener("click", (event) => {
  ...
  const item = state.drivers.find((driver) => driver.id === id);
...
if (!restoreDriversSyncStatus()) {
  setDriversSyncStatus(
    useSupabase ? "Shared driver sync ready." : "Local-only mode on this device.",
    useSupabase ? "neutral" : "local",
    { persist: false }
  );
}
