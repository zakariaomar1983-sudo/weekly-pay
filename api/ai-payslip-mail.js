...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (const summary of list.messages || []) {
      ...
      const prompt = `Extract a driver payslip from this Gmail message and its attachments. Inspect the full daily records table, not only the earnings summary. Use the daily rate or daily subtotal shown for the driver's ordinary work when calculating dailyRate. Identify night-run rows, drops, or night-run fees from the service codes, docket details, additional details, and daily records; calculate nightRunDrops and nightRunPay when the document provides enough information. Do not confuse total job earnings, fuel allowance, GST, or invoice total with the daily rate or night-run pay. Return JSON only with keys: driver, truckNumber, payPeriod, daysWorked, dailyRate, nightRunDrops, nightRunPay, driverBonus, deductions, paymentDate, netPay, confidence. Use numbers for numeric fields and null when unknown. Do not invent values. Email text:\n${source}`;
