...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (const summary of list.messages || []) {
      ...
      const prompt = `Extract a driver payslip from this Gmail message and its attachments. Return JSON only with keys: driver, truckNumber, payPeriod, daysWorked, dailyRate, nightRunDrops, nightRunPay, driverBonus, deductions, paymentDate, netPay, confidence. Use numbers for numeric fields and null when unknown. Do not invent values. Email text:\n${source}`;
